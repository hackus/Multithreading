...
class ChangeValueEngineWithAInteruptionTrick implements Engine {
    ...
    public ChangeValueEngineWithAInteruptionTrick(int maxIterations, SharedVariable shared){
        this.maxIterations = maxIterations;
        hasNextObject = shared;
    }

    public void put(Integer intValue){
        while(hasNextObject.getValue() && isPutFinalized() && !Thread.interrupted()){}
        putIterations++;
    ...
    public Integer take(){
        while(!hasNextObject.getValue() && isTakeFinalized() && !Thread.interrupted()){}
        takeIterations++;
